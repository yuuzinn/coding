...
`Map`에서 `get`은 시간복잡도(최악) `O(N)`이다. key값이 같은 인덱스를 가리키는 상황일 때, 내부에선 이 인덱스 내의 `리스트`를 순회해야하기 때문에 `O(N)`이다. O(1) 아님.

Map으로 초기화할 때는 기존에 new 를 통해 초기화하고 값을 for문으로 넣었지만, `stream API`에서 지원해주는 `toMap` 메서드를 통해 `Map` 객체로 초기화시킬 수 있음을 알게 됨. 자료구조 `Map`에 대해선 좀 더 공부하고, 활용할 수 있는 곳에선 적극적으로 활용해보는 연습을 해봐야겠음.
...
stream 에서 람다식으로 먼저 풀어보기, 그 다음으로 익숙해지면 메서드 참조형으로...


생각보다 자료구조의 응용이 상당히 중요하다!!!!!!! 여태 전부 `List`와 기본 for문으로 짜왔었는데.. 생각을 넓게 하는 버릇을 들이자
